...
Now, $f'(x)$ is monic, because it is a product of monic polynomials; it has degree less than $f(x)$, since $deg(r(x)) < deg(a_{1}(x))$ [ this is because at the right we have $b_{1}(x)q(x)$ which has the same degree of $a_{1}(x)$ and $deg(b_{2}(x) \dots b_{j}(x)) < deg(q(x)a_{2}(x) \dots a_{k}(x))$, also, we can safely consider every prime polynomial as a 1-degree polynomial ] and it has two different factorizations, with $b_{1}(x)$ a factor in one but not a divisor of any of the factors in the other, a contradiction.

There are other observations which could be made about this (nonsimple) theorem, but proceeding it will be quite intuitive that every monic polynomial generated by different prime polynomials is unique.
