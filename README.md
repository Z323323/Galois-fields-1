...
For any $\beta \in F_{q}^{\star}$, consider the cyclic subgroup $S(\beta) = \\{ 1, \beta, \beta^{2}, \dots, \\}$ of $F_{q}^{\star}$ generated by $\beta$. The size $|S(\beta)|$ of this subgroup is called the multiplicative order of $\beta$. By the cyclic group theorem, $\beta^{|S(\beta)|} = 1$, and by Lagrange's theorem, $|S(\beta)|$ must divide $|F_{q}^{\star}| = q - 1$. [ [https://github.com/Z323323/Group-theory-elements/blob/main/README.md#lagranges-theorem] fits the current scenario too because every remainder polynomial is actually coprime with the prime polynomial $g(x)$ and so we will have $q - 1$ coprime-with-g(x) polynomials in the field, and the Lagrange's theorem applies. ] It follows that $\beta^{q - 1} = 1$ for all $\beta \in F_{q}^{\star}$. In other words, every $\beta \in F_{q}^{\star}$ is a root of the polynomial equation $x^{q - 1} = 1$, or equivalently of the polynomial $x^{q - 1} - 1 \in F_{q}[x]$. By the polynomial roots theorem, $x^{q - 1} - 1$ can have at most $q - 1$ roots in $F_{q}$, so these are all the roots of $x^{q - 1} - 1$. Thus, $x^{q - 1} - 1$ factors into the product of the degree-1 polynomials $x - \beta$ for all $\beta \in F_{q}^{\star}$. Moreover, since $0 \in F_{q}$ is a root of the polynomial $x$ and $x(x^{q - 1} - 1) = x^{q} - x$, the polynomial $x^{q} - x$ factors into the product of the degree-1 polynomials $x - \beta$ for all $\beta \in F_{q}$.
